...
if [ -n "$host" ]; then
    sed -i.bak "s/^hostname: .*/hostname: $host/g" ./harbor.yml
    rm -f ./harbor.yml.bak  # Remove backup file if you don't need it
fi

# Comment out HTTPS section
sed -i.bak -e '/port: 443/ s/^/#/' ./harbor.yml  
sed -i.bak -e '/https:/ s/^/#/' ./harbor.yml  
sed -i.bak -e '/certificate:*/ s/^/#/' ./harbor.yml  
sed -i.bak -e '/private_key:*/ s/^/#/' ./harbor.yml  
rm -f ./harbor.yml.bak  # Remove backup files
